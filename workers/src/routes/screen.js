...
export async function screenRoutes(request, env, ctx, { path, jsonResponse, errorResponse }) {
  ...
  if (request.method === 'POST' && path.startsWith('/api/screen/tier3')) {
    try {
      ...
      // Backfill market_cap for stocks missing it (via Finnhub metrics)
      if (env.FINNHUB_API_KEY) {
        const missingMcap = await env.DB.prepare(
          "SELECT ticker FROM stocks WHERE market_cap IS NULL AND ticker NOT LIKE '\\_\\_%' ESCAPE '\\' LIMIT 40"
        ).all();
        if (missingMcap.results?.length > 0) {
          try {
            const { getBasicMetrics } = await import('../services/finnhub.js');
            let filled = 0;
            for (const row of missingMcap.results) {
              try {
                const metrics = await getBasicMetrics(row.ticker, env.FINNHUB_API_KEY);
                if (metrics.market_cap_m && metrics.market_cap_m > 0) {
                  await env.DB.prepare('UPDATE stocks SET market_cap = ? WHERE ticker = ?')
                    .bind(Math.round(metrics.market_cap_m), row.ticker).run();
                  filled++;
                }
              } catch (e) {
                if (e.message?.includes('rate limit') || e.message?.includes('429')) break;
              }
            }
            console.log(`Market cap backfill: ${filled}/${missingMcap.results.length} stocks updated`);
          } catch (e) {
